...
TYPE_PROC_REF(/obj/vertibird, toggleLock)(mob/user)
	if(locked)
		locked = FALSE
...
TYPE_PROC_REF(/obj/vertibird, getIn)(mob/U)
	src.visible_message("[U] enters the vertibird.")
	U.forceMove(GLOB.vertibirdEnterZone)

TYPE_PROC_REF(/obj/vertibird, moveIn)(obj/O)
	O.forceMove(GLOB.vertibirdEnterZone)

TYPE_PROC_REF(/obj/vertibird, ejectTurf)()
	return locate(src.x, src.y + 6, src.z)

TYPE_PROC_REF(/obj/vertibird, getLocationsHTML)()
	var/html
...
TYPE_PROC_REF(/obj/vertibird, flew)(targetX, targetY, targetZ)
...
TYPE_PROC_REF(/obj/vertibird, fly)(targetX, targetY, targetZ)
	if(inFly)
		return

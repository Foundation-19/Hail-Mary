...
fallout/code/datums/status_effects/chems.dm - handles the subject's reactions
...
/datum/reagent/fermi/enthrallExplo/on_mob_life(mob/living/carbon/M)//Love gas, only affects while it's in your system,Gives a positive moodlet if close, gives brain damagea and a negative moodlet if not close enough.
	...
	if(!M.has_status_effect(STATUS_EFFECT_INLOVE))
		...
		to_chat(M, "<span class='warning'>You suddenly feel like making friends with [love]. You feel strangely drawn towards them.</span>")
...
/datum/reagent/fermi/proc/FallInLove(mob/living/carbon/Lover, mob/living/carbon/Love)
	if(Lover.has_status_effect(STATUS_EFFECT_INLOVE))
		...
		return
	to_chat(Lover, "<span class='warning'>You develop a deep and sudden bond with [Love]. You are determined to keep them safe and happy, and feel drawn towards them.</span>")
	if(Lover.mind)

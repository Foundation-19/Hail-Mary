TYPE_PROC_REF(/client, mentor_follow)(mob/living/M)
	if(!is_mentor())
		return
	var/orbiting = TRUE
	if(!isobserver(usr))
		...
		add_verb(usr, TYPE_PROC_REF(/client, mentor_unfollow))
...
TYPE_PROC_REF(/client, mentor_unfollow)()
	set category = "Mentor"
	...
	remove_verb(usr, TYPE_PROC_REF(/client, mentor_unfollow))

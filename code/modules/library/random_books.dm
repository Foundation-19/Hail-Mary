...
/proc/create_random_books(amount = 2, location, fail_loud = FALSE, category = null)
	...
	if(query_get_random_books.Execute())
		while(query_get_random_books.NextRow())
			...
			. += B
			B.author	=	unsanitizeSQL(query_get_random_books.item[2])
			B.title		=	unsanitizeSQL(query_get_random_books.item[3])
			B.dat		=	unsanitizeSQL(query_get_random_books.item[4])
			B.name		=	"Book: [B.title]"

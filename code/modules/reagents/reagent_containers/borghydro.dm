...
/obj/item/reagent_containers/borghypo/borgshaker/DescribeContents()
	...
	var/datum/reagent/R = locate() in RS.reagent_list
	if(R)
		to_chat(usr, span_notice("It currently has [R.volume] unit\s of [R.name] stored."))
		empty = 0

	if(empty)
		to_chat(usr, span_warning("It is currently empty! Please allow some time for the synthesizer to produce more."))

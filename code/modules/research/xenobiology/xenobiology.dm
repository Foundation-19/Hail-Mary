...
/obj/item/slime_extract/attack(mob/living/simple_animal/slime/M, mob/user)
	...
	if(M.stat)
		...
		return
	if(!M.is_adult)
		to_chat(user, span_warning("The slime must be an adult to cross it's core!"))
		return
...
/obj/item/slime_extract/pyrite/activate(mob/living/carbon/human/user, datum/species/jelly/luminescent/species, activation_type)
	switch(activation_type)
		if(SLIME_ACTIVATE_MINOR)
			var/blacklisted_cans = list(/obj/item/toy/crayon/spraycan/borg)
...
/obj/item/slimepotion/genderchange/attack(mob/living/L, mob/user)
	...
	if(!L.set_gender(L.gender == MALE ? FEMALE : MALE))
		to_chat(user,span_warning("It won't work on [L]!"))
		return
	qdel(src)
...
/obj/item/slimepotion/slime/slimeradio/attack(mob/living/M, mob/user)
	...
	if(!M.can_be_implanted())
		to_chat(user, span_warning("[M] is incompatible with the potion!"))
		return
	if(M.stat)
		to_chat(user, span_warning("[M] is dead!"))
		return

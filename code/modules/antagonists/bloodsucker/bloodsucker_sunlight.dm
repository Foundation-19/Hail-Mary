...
TYPE_PROC_REF(/obj/effect/sunlight, warn_daylight)(danger_level =0, vampwarn = "", vassalwarn = "")
	for(var/datum/mind/M in SSticker.mode.bloodsuckers)
		...
		if(M.current)
			if(danger_level == 1)
				M.current.playsound_local(null, 'sound/runtime/chatter/griffin_3.ogg', 50 + danger_level, 1)
			else if(danger_level == 2)
				M.current.playsound_local(null, 'sound/runtime/chatter/griffin_5.ogg', 50 + danger_level, 1)

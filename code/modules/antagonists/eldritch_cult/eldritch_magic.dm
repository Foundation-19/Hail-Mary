...
/obj/item/melee/touch_attack/mansus_fist/afterattack(atom/target, mob/user, proximity_flag, click_parameters)
	...
	if(ishuman(target))
		...
		if(tar.anti_magic_check())
			tar.visible_message(span_danger("Spell bounces off of [target]!"),span_danger("The spell bounces off of you!"))
			return ..()
	...
	for(var/turf/T in turfs)
		...
		for(var/mob/living/L in T.contents)
			if(L.anti_magic_check())
				L.visible_message(span_danger("Spell bounces off of [L]!"),span_danger("The spell bounces off of you!"))
				continue
			if(L in hit_list || L == source)
				continue
			hit_list += L
			L.adjustFireLoss(15)
			to_chat(L, span_userdanger("You're hit by a blast of fire!"))
...
/obj/effect/proc_holder/spell/pointed/trigger/mute/eldritch
	...
	invocation_type = "whisper"
	message = span_userdanger("It feels as if your tongue is being held down by an unseen force!")

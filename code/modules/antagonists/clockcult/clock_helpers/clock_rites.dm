...
TYPE_PROC_REF(/datum/clockwork_rite, try_cast)(obj/effect/clockwork/sigil/rite/R, mob/living/invoker) //Performs a ton of checks to see if the invoker can cast the rite
	if(!istype(R))
		return FALSE
	if(!R || !R.loc)
		return FALSE
...
TYPE_PROC_REF(/datum/clockwork_rite, cast)(mob/living/invoker, turf/T, mob/living/carbon/human/target) //Casts the rite and uses up ingredients. Doublechecks some things to prevent bypassing some restrictions via funky timing or badminnery.
	if(!T || !invoker)
		return FALSE
...
TYPE_PROC_REF(/datum/clockwork_rite, post_cast)(cast_succeeded)
	if(cast_succeeded)
		times_used++
	return TRUE

TYPE_PROC_REF(/datum/clockwork_rite, build_info)() //Constructs the info text of a given rite, based on the vars of the rite
	. = ""

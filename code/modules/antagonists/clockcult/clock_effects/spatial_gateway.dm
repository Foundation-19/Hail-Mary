...
/obj/effect/clockwork/spatial_gateway/examine(mob/user)
	...
	if(is_servant_of_ratvar(user) || isobserver(user))
		. += span_brass(" [is_stable ? "It is stabilised and can be used as much as is neccessary." : "It has [uses] use\s remaining."]")
...
/atom/movable/proc/procure_gateway(mob/living/invoker, time_duration, gateway_uses, two_way)
	...
	if(issrcobelisk && istargetobelisk && src.z != target.z && (is_reebe(src.z) || is_reebe(target.z)))
		...
		span_brass("With a word, you rip open a stable two-way rift between reebe and the mortal realm."))
		...
		stable_S2.visible_message(span_warning("The air in front of [target] ripples before suddenly tearing open!"))
	else
		invoker.visible_message(span_warning("The air in front of [invoker] ripples before suddenly tearing open!"), \
...
/obj/effect/clockwork/spatial_gateway/stable/attackby(obj/item/I, mob/living/user, params)
	...
	user.visible_message(span_warning("The rift begins to ripple as [user] points [user.p_their()] slab at it!"), span_brass(" You begin to shutdown the stabilised gateway with your slab."))
...
/obj/effect/clockwork/spatial_gateway/stable/proc/start_shutdown()
		...
		src.visible_message(span_warning("[src] begins to destabilise!"))
		linked_gateway.visible_message(span_warning("[linked_gateway] begins to destabilise!"))

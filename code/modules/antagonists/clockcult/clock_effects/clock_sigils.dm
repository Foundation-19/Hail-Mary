...
/obj/effect/clockwork/sigil/transgression/sigil_effects(mob/living/L)
	...
	span_userdanger("[target_flashed ? "An unseen force":"The glowing sigil around you"] [iscultist(L) ? "painfully bursts into flames!" : "holds you in place!"]"))
...
/obj/effect/clockwork/sigil/transmission/ex_act(severity)
	if(severity == 3)
		...
		visible_message(span_warning("[src] flares a brilliant orange!"))
	else
		..()
...
/obj/effect/clockwork/sigil/transmission/proc/charge_cyborg(mob/living/silicon/robot/cyborg)
	...
	if(adjust_clockwork_power(-giving_power))
		cyborg.visible_message(span_warning("[cyborg] glows a brilliant orange!"))
...
/obj/effect/clockwork/sigil/transmission/proc/cyborg_checks(mob/living/silicon/robot/cyborg, silent)
	...
	if(!get_clockwork_power())
		if(!silent)
			to_chat(cyborg, span_warning("There is no power available across sigils!"))
		return FALSE
			...
			if(L.stat == DEAD)
				...
				if(GLOB.clockwork_vitality >= revival_cost && (ghost || (L.mind && L.mind.active)))
					if(L.has_status_effect(STATUS_EFFECT_ICHORIAL_STAIN))
						visible_message(span_boldwarning("[src] strains, but nothing happens..."))
						if(L.pulledby)
							to_chat(L.pulledby, span_userdanger("[L] was already revived recently by a vitality matrix! Wait a bit longer!"))
						break
					else
						if(ghost)

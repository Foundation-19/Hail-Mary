...
TYPE_PROC_REF(/obj/machinery/atmospherics/components/unary/vent_pump, set_frequency)(new_frequency)
	SSradio.remove_object(src, frequency)
	frequency = new_frequency
	if(frequency)
		radio_connection = SSradio.add_object(src, frequency,radio_filter_in)

TYPE_PROC_REF(/obj/machinery/atmospherics/components/unary/vent_pump, broadcast_status)()
	if(!radio_connection)
		return
...
/obj/machinery/atmospherics/components/unary/vent_pump/welder_act(mob/living/user, obj/item/I)
	...
	if(I.use_tool(src, user, 20, volume=50))
		if(!welded)
			user.visible_message("[user] welds the vent shut.", span_notice("You weld the vent shut."), span_italic("You hear welding."))
			welded = TRUE
		else
			user.visible_message("[user] unwelded the vent.", span_notice("You unweld the vent."), span_italic("You hear welding."))
			welded = FALSE
		update_icon()

 ...
 */
TYPE_PROC_REF(/datum/song, compile_legacy)()
	if(!length(src.lines))
		return
 ...
 */
TYPE_PROC_REF(/datum/song, playkey_legacy)(note, acc as text, oct, mob/user)
	...
	for(var/i in hearing_mobs)
		var/mob/M = i
		M.playsound_local(source, null, volume * using_instrument.volume_multiplier, falloff_distance = 5, S = music_played)

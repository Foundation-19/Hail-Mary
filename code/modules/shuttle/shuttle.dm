	...
	if(distant_source)
		for(var/mob/M in SSmobs.clients_by_zlevel[z])
			...
			if(dist_far <= long_range && dist_far > range)
				M.playsound_local(distant_source, "sound/effects/[selected_sound]_distance.ogg", 100, falloff_distance = 20)
				...
				M.playsound_local(source, "sound/effects/[selected_sound].ogg", 100, falloff_distance = range / 2)

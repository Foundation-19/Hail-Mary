	...
	if(isethereal(user))
		...
		if(H.a_intent == INTENT_HARM)
			if(cell.charge <= (cell.maxcharge / 2)) // if charge is under 50% you shouldnt drain it
				to_chat(H, span_warning("The APC doesn't have much power, you probably shouldn't drain any."))
				return
...
TYPE_PROC_REF(/obj/machinery/power/apc, hijack)(mob/living/L)
	if (!istype(L))
		return
	if(being_hijacked)
		to_chat(L, span_warning("This APC is already being hijacked!"))
		return
	if (hijacker && hijacker != L)
		var/obj/item/implant/hijack/H = L.getImplant(/obj/item/implant/hijack)
		to_chat(L, span_warning("Someone already has control of this APC. Beginning counter-hijack."))
		H.hijacking = TRUE
		being_hijacked = TRUE
		if (do_after(L,20 SECONDS,target=src))
			...
			to_chat(L, span_warning("Aborting."))
			H.hijacking = FALSE
			being_hijacked = FALSE
			return
	to_chat(L, span_notice("Beginning hijack of APC."))
	var/obj/item/implant/hijack/H = L.getImplant(/obj/item/implant/hijack)
	H.hijacking = TRUE
	being_hijacked = TRUE

...
TYPE_PROC_REF(/datum/netdata, pre_send)(datum/component/ntnet_interface/interface)
	// Decrypt the passkey.
	if(autopasskey)
...
TYPE_PROC_REF(/datum/netdata, standard_format_data)(primary, secondary, passkey)
	data["data"] = primary
	data["data_secondary"] = secondary
	data["encrypted_passkey"] = passkey

TYPE_PROC_REF(/datum/netdata, json_to_data)(json)
	data = json_decode(json)

TYPE_PROC_REF(/datum/netdata, json_append_to_data)(json)
	data |= json_decode(json)

TYPE_PROC_REF(/datum/netdata, data_to_json)()
	return json_encode(data)

TYPE_PROC_REF(/datum/netdata, json_list_generation_admin)()	//for admin logs and such.
	. = list()
	. |= json_list_generation()

TYPE_PROC_REF(/datum/netdata, json_list_generation)()
	. = list()
	. |= json_list_generation_netlog()
	.["network_id"] = network_id

TYPE_PROC_REF(/datum/netdata, json_list_generation_netlog)()
	. = list()
	.["recipient_ids"] = recipient_ids
	.["sender_id"] = sender_id
	.["data_list"] = data

TYPE_PROC_REF(/datum/netdata, generate_netlog)()

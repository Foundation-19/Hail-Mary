...
TYPE_PROC_REF(/obj/item/organ/vocal_cords, can_speak_with)() //if there is any limitation to speaking with these cords
	return TRUE

TYPE_PROC_REF(/obj/item/organ/vocal_cords, speak_with)(message) //do what the organ does
	return

TYPE_PROC_REF(/obj/item/organ/vocal_cords, handle_speech)(message) //actually say the message
...
/proc/velvetspeech(message, mob/living/user, base_multiplier = 1, message_admins = FALSE, debug = FALSE)
	...
	if(findtext(message, reward_words))
		for(var/V in listeners)
			...
			if(L == user)
				continue
			if (E.lewd)
				addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), L, span_love("[E.enthrallGender] has praised me!!")), 5)
	...
	//PUNISH mixable  works
		...
		for(var/V in listeners)
			...
			if (E.lewd)
				...
				addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), L, "<span class='warning'>I've failed [E.master]...</b></span>"), 5)
				E.resistanceTally += power_multiplier
				E.enthrallTally += power_multiplier
				E.cooldown += 1
	...
	//FORGET (A way to cancel the process)
		...
		for(var/mob/living/carbon/C in listeners)
			var/datum/status_effect/chem/enthrall/E = C.has_status_effect(/datum/status_effect/chem/enthrall)
			if(E.phase == 4)
				addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), C, "<span class='warning'>You're unable to forget about [(E.lewd?"the dominating presence of [E.enthrallGender]":"[E.master]")]!</b></span>"), 5)
				continue
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					...
					addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), H, "<span class='[(E.lewd?"love":"warning")]'>Before you can even think about it, you quickly remove your clothes in response to [(E.lewd?"your [E.enthrallGender]'s command'":"[E.master]'s directive'")].</b></span>"), 5)
					E.cooldown += 10
		...
		for(var/V in listeners)
			...
			switch(E.phase)
				if(2 to INFINITY)
					L.lay_down()
					E.cooldown += 10
					addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), L, "[(E.lewd?"<span class='love'>You eagerly lie down!":"<span class='notice'>You suddenly lie down!")]</b></span>"), 5)
	...
	//STATE TRIGGERS
		...
		for(var/V in listeners)
			...
			if (E.phase == 3)
				...
				addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), C, span_notice("You whisper your triggers to [(E.lewd?"Your [E.enthrallGender]":"[E.master]")].")), 5)
				...
				if(!E.lewd)
					...
					if (E.mental_capacity >= 5)
						...
						if ((findtext(trigger2, custom_words_words)))
							...
							E.mental_capacity -= 5
							addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), H, span_notice("[(E.lewd?"your [E.enthrallGender]":"[E.master]")] whispers you a new trigger.")), 5)
			...
			if(E.phase == 3)
				if (get_dist(user, H) > 1)//Requires user to be next to their pet.
					...
					if (E.mental_capacity >= 200)
						...
						addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), H, span_notice("[(E.lewd?"Your [E.enthrallGender]":"[E.master]")] whispers you a new objective.")), 5)
		...
		for(var/V in listeners)
			...
			if(E.phase > 1)
				if(user.ckey == E.enthrallID && user.real_name == E.master.real_name)
					E.master = user
					addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), H, "<span class='nicegreen'>[(E.lewd?"You hear the words of your [E.enthrallGender] again!! They're back!!":"You recognise the voice of [E.master].")]</b></span>"), 5)

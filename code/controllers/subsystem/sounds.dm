...
TYPE_PROC_REF(/datum/controller/subsystem/sounds, setup_available_channels)()
	available_channels = list()
	for(var/i in 1 to using_channels_max)
		available_channels[num2text(i)] = TRUE

/// Removes a channel from using list.
TYPE_PROC_REF(/datum/controller/subsystem/sounds, free_sound_channel)(channel)
...
TYPE_PROC_REF(/datum/controller/subsystem/sounds, free_datum_channels)(datum/D)
...
TYPE_PROC_REF(/datum/controller/subsystem/sounds, free_datumless_channels)()
	free_datum_channels(DATUMLESS)

/// NO AUTOMATIC CLEANUP - If you use this, you better manually free it later! Returns an integer for channel.
TYPE_PROC_REF(/datum/controller/subsystem/sounds, reserve_sound_channel_datumless)()
...
TYPE_PROC_REF(/datum/controller/subsystem/sounds, reserve_sound_channel)(datum/D)
...
TYPE_PROC_REF(/datum/controller/subsystem/sounds, random_available_channel_text)()
	return pick(available_channels)

/// Random available channel, returns number
TYPE_PROC_REF(/datum/controller/subsystem/sounds, random_available_channel)()
	return text2num(pick(available_channels))

/// If a channel is available
TYPE_PROC_REF(/datum/controller/subsystem/sounds, is_channel_available)(channel)
	return available_channels[num2text(channel)]

/// How many channels we have left.
TYPE_PROC_REF(/datum/controller/subsystem/sounds, available_channels_left)()

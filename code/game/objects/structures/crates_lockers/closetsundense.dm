/obj/structure/closetundense
	...
	var/mob_storage_capacity = 3 // how many human sized mob/living can fit together inside a closetundense.
	...
	var/eigen_teleport = FALSE //If the closetundense leads to Mr Tumnus.
	var/obj/structure/closetundense/eigen_target //Where you go to.
...
/obj/structure/closetundense/Initialize(mapload)
	. = ..()
	update_icon()
...
/obj/structure/closetundense/proc/PopulateContents()
	return

/obj/structure/closetundense/Destroy()
	dump_contents(override = FALSE)
	return ..()

/obj/structure/closetundense/update_icon()
	. = ..()
	if(!opened)
		layer = TRAY_LAYER
	else
...
/obj/structure/closetundense/update_overlays()
	. = ..()
	closetundense_update_overlays(.)

/obj/structure/closetundense/proc/closetundense_update_overlays(list/new_overlays)
	. = new_overlays
	if(!opened)
		if(icon_door)
			. += "[icon_door]_door"
...
/obj/structure/closetundense/examine(mob/user)
	. = ..()
	if(welded)
...
/obj/structure/closetundense/proc/can_open(mob/living/user)
	if(welded || locked)
		return FALSE
...
/obj/structure/closetundense/proc/can_close(mob/living/user)
	var/turf/T = get_turf(src)
	for(var/obj/structure/closetundense/closetundense in T)
		if(closetundense != src && !closetundense.wall_mounted)
			return FALSE
	for(var/mob/living/L in T)
		if(L.move_resist >= MOVE_FORCE_VERY_STRONG || horizontal && L.mob_size > MOB_SIZE_TINY && L.density)
			if(user)
...
/obj/structure/closetundense/proc/can_lock(mob/living/user, check_access = TRUE) //set check_access to FALSE if you only need to check if a locker has a functional lock rather than access
	if(!secure)
		return FALSE
	if(broken)
...
/obj/structure/closetundense/proc/togglelock(mob/living/user)
...
/obj/structure/closetundense/proc/dump_contents(override = TRUE) //Override is for not revealing the locker electronics when you open the locker, for example
	var/atom/L = drop_location()
	for(var/atom/movable/AM in src)
		if(AM == lockerelectronics && override)
			continue
		AM.forceMove(L)
		if(throwing) // you keep some momentum when getting out of a thrown closetundense
			step(AM, dir)
	if(throwing)
		throwing.finalize(FALSE)

/obj/structure/closetundense/proc/take_contents()
	var/atom/L = drop_location()
	for(var/atom/movable/AM in L)
		if(AM != src && insert(AM) == -1) // limit reached
			break

/obj/structure/closetundense/proc/open(mob/living/user)
...
/obj/structure/closetundense/proc/insert(atom/movable/AM)
...
/obj/structure/closetundense/proc/insertion_allowed(atom/movable/AM)
	if(ismob(AM))
		if(!isliving(AM)) //let's not put ghosts or camera mobs inside closetundenses...
			return FALSE
		var/mob/living/L = AM
	...
	else if(istype(AM, /obj/structure/closetundense))
		return FALSE
...
/obj/structure/closetundense/proc/close(mob/living/user)
...
/obj/structure/closetundense/proc/toggle(mob/living/user)
	if(opened)
		return close(user)
	else
		return open(user)

/obj/structure/closetundense/proc/bust_open()
	welded = FALSE //applies to all lockers
	locked = FALSE //applies to critter crates and secure lockers only
	broken = TRUE //applies to secure lockers only
	open()

/obj/structure/closetundense/proc/handle_lock_addition(mob/user, obj/item/electronics/airlock/E)
	add_fingerprint(user)
	if(lock_in_use)
...
/obj/structure/closetundense/proc/handle_lock_removal(mob/user, obj/item/screwdriver/S)
	if(lock_in_use)
		to_chat(user, span_notice("Wait for work on [src] to be done first!"))
		return
	if(locked)
...
/obj/structure/closetundense/deconstruct(disassembled = TRUE)
	if(ispath(material_drop) && material_drop_amount && !(flags_1 & NODECONSTRUCT_1))
		new material_drop(loc, material_drop_amount)
	qdel(src)

/obj/structure/closetundense/obj_break(damage_flag)
	if(!broken && !(flags_1 & NODECONSTRUCT_1))
		bust_open()

/obj/structure/closetundense/attackby(obj/item/W, mob/user, params)
	if(user in src)
		return
...
/obj/structure/closetundense/proc/tool_interact(obj/item/W, mob/user)//returns TRUE if attackBy call shouldnt be continued (because tool was used/closetundense was of wrong type), FALSE if otherwise
	. = TRUE
...
/obj/structure/closetundense/proc/after_weld(weld_state)
	return

/obj/structure/closetundense/MouseDrop_T(atom/movable/O, mob/living/user)
...
/obj/structure/closetundense/relaymove(mob/user)
...
/obj/structure/closetundense/on_attack_hand(mob/user, act_intent = user.a_intent, unarmed_attack_flags)
...
/obj/structure/closetundense/attack_paw(mob/user)
	return attack_hand(user)

/obj/structure/closetundense/attack_robot(mob/user)
	if(user.Adjacent(src))
		return attack_hand(user)

// tk grab then use on self
/obj/structure/closetundense/attack_self_tk(mob/user)
	return attack_hand(user)

/obj/structure/closetundense/verb/verb_toggleopen()
	set src in oview(1)
	set category = "Object"
...
/obj/structure/closetundense/container_resist(mob/living/user)
	...
	//okay, so the closetundense is either welded or locked... resist!!!
...
/obj/structure/closetundense/AltClick(mob/user)
	. = ..()
...
/obj/structure/closetundense/CtrlShiftClick(mob/living/user)
...
/obj/structure/closetundense/emag_act(mob/user)
	. = ..()
	if(!secure || broken)
		return
...
/obj/structure/closetundense/get_remote_view_fullscreens(mob/user)
	if(user.stat == DEAD || !(user.sight & (SEEOBJS|SEEMOBS)))
		user.overlay_fullscreen("remote_view", /obj/screen/fullscreen/impaired, 1)

/obj/structure/closetundense/emp_act(severity)
	. = ..()
	if(. & EMP_PROTECT_SELF)
		return
...
/obj/structure/closetundense/contents_explosion(severity, target)
	for(var/atom/A in contents)
		A.ex_act(severity, target)
		CHECK_TICK

/obj/structure/closetundense/singularity_act()
	dump_contents()
	..()

/obj/structure/closetundense/AllowDrop()
	return TRUE


/obj/structure/closetundense/return_temperature()
	return

/obj/structure/closetundense/proc/dive_into(mob/living/user)
...
/obj/structure/closetundense/anchored //For mappers to easily placed anchored closetundenses
	anchored = TRUE
	storage_capacity = 30

//Fallout closetundenses
...
/obj/structure/closetundense/locker/higherlayer/update_icon()
	. = ..()
	if(!opened)
		layer = BELOW_OBJ_LAYER
	else
...
/obj/structure/closetundense/locker/oldstyle  //modded Eris sprite
	icon_state = "oldstyle"

/obj/structure/closetundense/locker/medcabinet
...
/obj/structure/closetundense/locker/fridge
	name = "fridge"

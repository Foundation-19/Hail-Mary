...
GLOBAL_LIST_INIT(metal_recipes, list ( \
	...
	new /datum/stack_recipe_list("lamp posts", list( \
		new /datum/stack_recipe("lamp post (singles)", /obj/structure/light_construct/lamp_post, 5, time = 25, one_per_turf = TRUE, on_floor = TRUE), \
		new /datum/stack_recipe("lamp post (doubles)", /obj/structure/light_construct/lamp_post/doubles, 5, time = 25, one_per_turf = TRUE, on_floor = TRUE), \
		new /datum/stack_recipe("lamp post (doubles/corner)", /obj/structure/light_construct/lamp_post/doubles/bent, 5, time = 25, one_per_turf = TRUE, on_floor = TRUE), \
		new /datum/stack_recipe("lamp post (triples)", /obj/structure/light_construct/lamp_post/triples, 5, time = 25, one_per_turf = TRUE, on_floor = TRUE), \
		new /datum/stack_recipe("lamp post (quadra)", /obj/structure/light_construct/lamp_post/quadra, 5, time = 25, one_per_turf = TRUE, on_floor = TRUE), \
	)), \
	null, \
...
//				span_italic("You hear wood carving.")) didnt work on this Db for some reason
		// 5 Second Timer
		if(!do_after(user, 5 SECONDS, TRUE, src))
			to_chat(user,"<span class='spider'><b>You didn't finish your carving.</b></span>")
			return TRUE
		// Make stick
...
/obj/item/stack/sheet/hay/suicide_act(mob/living/carbon/user)
	...
	addtimer(CALLBACK(user, TYPE_PROC_REF(/mob, gib)), 30)
	return BRUTELOSS

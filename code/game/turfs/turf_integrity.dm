...
/turf/attack_animal(mob/living/simple_animal/M)
	var/melee_damage = M.melee_damage_upper + M.melee_damage_lower / 2
	if (!can_hit)
		return ..()
	if(!melee_damage && !M.obj_damage)
		...
		if(M.obj_damage)
			...
			. = attack_generic(M, melee_damage, M.melee_damage_type, "melee", play_soundeffect, M.armour_penetration)
		if(. && !play_soundeffect)
			playsound(src, 'sound/effects/meteorimpact.ogg', 100, 1)

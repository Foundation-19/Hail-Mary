...
TYPE_PROC_REF(/obj/machinery/quantumpad, doteleport)(mob/user, obj/machinery/quantumpad/target_pad = linked_pad)
	if(target_pad)
		...
		spawn(teleport_speed)
			...
			flick("qpad-beam", src)
			playsound(get_turf(src), 'sound/weapons/emitter2.ogg', 25, 1, extrarange = 3, falloff_distance = 5)
			flick("qpad-beam", target_pad)
			playsound(get_turf(target_pad), 'sound/weapons/emitter2.ogg', 25, 1, extrarange = 3, falloff_distance = 5)

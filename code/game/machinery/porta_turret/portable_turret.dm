/obj/machinery/porta_turret
	...
	/// List of registered_name strings that this turret will not shoot.
	/// Populated by swiping ID cards on a linked terminal.
	var/list/id_whitelist = null
	/// Whether whitelist enforcement is active.
	/// When FALSE the whitelist is stored but ignored.
	var/whitelist_active = FALSE
...
/// Returns TRUE if the given mob is on this turret's active whitelist.
/// Call this in your target-acquisition check to skip whitelisted players.
/obj/machinery/porta_turret/proc/is_whitelisted(mob/living/target)
	if(!whitelist_active || !id_whitelist || !id_whitelist.len)
		return FALSE
	if(!istype(target))
		return FALSE
	// Check real_name (player character name)
	var/check_name = ishuman(target) ? target.real_name : target.name
	return (check_name in id_whitelist)

// ── Hook into target validation.
// Override this proc in your turret subtype if you have a custom one.
// The base proc returns TRUE if the turret should consider this mob a valid target.
// We extend it to respect the whitelist.
/obj/machinery/porta_turret/proc/valid_target(mob/living/target)

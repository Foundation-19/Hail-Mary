...
/obj/machinery/recharger/pump/RefreshParts()
	for(var/obj/item/stock_parts/capacitor/C in component_parts)
		recharge_coeff = C.rating

/obj/machinery/recharger/pump/examine(mob/user)
	. = ..()
	if(!in_range(user, src) && !issilicon(user) && !isobserver(user))
		. += span_warning("You're too far away to examine [src]'s contents and display!")
		return
...
/obj/machinery/recharger/pump/on_attack_hand(mob/user, act_intent = user.a_intent, unarmed_attack_flags)
...
/obj/machinery/recharger/pump/attack_tk(mob/user)
	if(charging)
		charging.update_icon()
		charging.forceMove(drop_location())

/obj/machinery/recharger/pump/process()
	if(stat & (NOPOWER|BROKEN) || !anchored)
		return PROCESS_KILL
...
/obj/machinery/recharger/pump/power_change()
	..()
	update_icon()

/obj/machinery/recharger/pump/emp_act(severity)
	. = ..()
	if (. & EMP_PROTECT_CONTENTS)
		return
	if(!(stat & (NOPOWER|BROKEN)) && anchored)
...
/obj/machinery/recharger/pump/update_icon_state()
	if(stat & (NOPOWER|BROKEN) || !anchored)
		icon_state = "rustpipe-vent"
	else if(panel_open)
		icon_state = "rustpipe-vent"
		...
		if(using_power)
			icon_state = "rustpipe-ventcharge1"
		else
			icon_state = "rustpipe-ventcharge2"
	else
		icon_state = "rustpipe-vent"

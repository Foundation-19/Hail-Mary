...
TYPE_PROC_REF(/datum/brahmin_fashion, apply)(mob/living/simple_animal/cow/brahmin/D) //none of this currently seems to work, but the backpacks go on so I'm content for now
	if(name)
		D.name = name
	if(desc)
		D.desc = desc
...
TYPE_PROC_REF(/datum/brahmin_fashion, get_overlay)(dir)

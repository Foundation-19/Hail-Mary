...
/datum/wound/blunt/get_scanner_description(mob/user)
	...
	if(!gelled)
		...
		. += span_notice("Continue Alternative Treatment: Apply surgical tape directly to injured limb to begin bone regeneration. Note, this is both excruciatingly painful and slow.")
	else
		. += span_notice("Note: Bone regeneration in effect. Bone is [round(regen_points_current*100/regen_points_needed)]% regenerated.")
